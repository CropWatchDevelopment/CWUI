<script lang="ts">
	...
	const icons = {
		...
		settings: 'M8 10a2 2 0 100-4 2 2 0 000 4zM12.5 8a4.5 4.5 0 01-.3 1.6l1.3 1.3-1.1 1.1-1.3-1.3A4.5 4.5 0 018 12.5a4.5 4.5 0 01-3.1-1.8L3.5 12l-1-1.1 1.3-1.3A4.5 4.5 0 013.5 8c0-.6.1-1.1.3-1.6L2.5 5.1l1.1-1.1 1.3 1.3A4.5 4.5 0 018 3.5c1.2 0 2.3.5 3.1 1.3l1.3-1.3 1.1 1.1-1.3 1.3c.2.5.3 1 .3 1.6z',
	...
	const demoItems: CwSideNavItem[] = [
		{ id: 'home', label: 'Dashboard', icon: icons.home, href: '#', active: true },
		{ id: 'analytics', label: 'Analytics', icon: icons.chart, href: '#' },
		{ id: 'users', label: 'Users', icon: icons.users, href: '#' },
		{ id: 'notifications', label: 'Notifications', icon: icons.bell, href: '#', separator: true },
		{ id: 'files', label: 'Files', icon: icons.folder, href: '#' },
		{ id: 'settings', label: 'Settings', icon: icons.settings, href: '#', separator: true },
		{ id: 'disabled', label: 'Archived', icon: icons.folder, disabled: true }
	];
...
<h2>CwSideNav</h2>
<p class="demo-desc">Collapsible side navigation with open, mini, and hidden modes. Supports left/right positioning, header, footer, and icon-only mini mode.</p>

<div class="demo-controls">
	<div class="demo-controls__group">
		<span class="demo-controls__label">Mode</span>
		<CwButton size="sm" variant={mode === 'open' ? 'primary' : 'secondary'} onclick={() => (mode = 'open')}>Open</CwButton>
		<CwButton size="sm" variant={mode === 'mini' ? 'primary' : 'secondary'} onclick={() => (mode = 'mini')}>Mini</CwButton>
		<CwButton size="sm" variant={mode === 'hidden' ? 'primary' : 'secondary'} onclick={() => (mode = 'hidden')}>Hidden</CwButton>
	</div>
	<div class="demo-controls__group">
		<span class="demo-controls__label">Side</span>
		<CwButton size="sm" variant={side === 'left' ? 'primary' : 'secondary'} onclick={() => (side = 'left')}>Left</CwButton>
		<CwButton size="sm" variant={side === 'right' ? 'primary' : 'secondary'} onclick={() => (side = 'right')}>Right</CwButton>
	</div>
</div>
...
<div class="demo-frame" class:demo-frame--reversed={side === 'right'}>
	...
	>
		...
		{/snippet}
		{#snippet headerMini()}
			<svg viewBox="0 0 24 24" fill="none" style="width:1.25rem;height:1.25rem;color:var(--cw-accent)">
